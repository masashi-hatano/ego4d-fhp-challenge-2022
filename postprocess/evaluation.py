...
def calculate_L2_loss(preds, labels, right_list, left_list, right_final_list, left_final_list):
    ...
    for key_frame in range(5):
        ...
        for single_hand in range(2):
            if mask[key_frame*4+single_hand*2]:
                cd_pred = (int(pred[key_frame*4+single_hand*2]), int(pred[key_frame*4+single_hand*2+1]))
                cd_label = (int(label[key_frame*4+single_hand*2]), int(label[key_frame*4+single_hand*2+1]))
...
def main(cfg, num_vis, eval_type, plot):
    ...
    cu.load_checkpoint(
        checkpoint, model, cfg.NUM_GPUS > 1
    )
